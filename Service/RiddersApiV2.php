...
class RiddersApiV2 extends AbstractService implements ServiceInterface
{
    const BASE_URI = 'https://postcodev2.ridders.nl/';
    ...
    {
        $apiKey = $this->scopeConfig->getValue('ridders_postcode/general/ridders_api_v2/api_key');
        if (!$apiKey) {
    ...
    public function getAddress(AddressInterface $address): AddressInterface
    {
        ...
        if (isset($content['errorCode'])) {
            switch ($content['errorCode']) {
                ...
                case 103:
                    throw new \Ridders\Postcode\Exception\ApiKeyInvalidException('API key is invalid');
                case 300:
                case 404:
